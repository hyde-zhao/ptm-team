...
公共因子库主库不存放在特性项目 `kym/` 或 `mfq/` 中。主库由 `ptm-team` 归档在仓库级 `resource/factor-libraries/`，安装后位于 `~/.ptm-team/resource/factor-libraries/` 或 `PTM_TEAM_RESOURCE_HOME/factor-libraries/`。
...
`kym/scenarios/` 下的部署型场景产物必须保留 Scenario Details 与结构化补充两层信息。
...
`kym/scenarios/confirmed-scenarios.md` 是真实组网对象的回链基线，至少保留：
...
- 去除路径分隔符和控制字符；
- 同名冲突追加 `-<LC-ID>`；
- 不创建 `ppdcs/<module>/<sub-module>/` 深目录。

### PC 拓扑物化字段

`ppdcs/pc/<basename>.md` 中若出现真实组网对象，必须同时保留以下字段：
...
| `source` | 来源基线，必须能回链到 `kym/scenarios/confirmed-scenarios.md` |
