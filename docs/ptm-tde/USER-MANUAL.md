...
> 运行时目录规则：**读 `input/`，按阶段写入项目根目录下的 `analysis/`、`design/`、`checkpoints/`、`delivery/`、`doc/`**
...
| 平台集成人员 | 通过 ptm-team 管理 ptm-tde 的安装、投影、升级和卸载 | 使用 ptm-team 统一工具完成组件管理 |

## 3. 组件与运行约定

### 3.1 安装入口归属

`ptm-tde` 后续作为 `ptm-team` 的一个组件交付，不再独立维护安装入口、project/user scope 投影脚本或卸载流程。安装、升级、卸载、平台目录投影和 dry-run 均由 `ptm-team` 统一控制。
...
推荐一个特性一个项目，直接在该项目根目录内完成需求分析和用例设计。推荐目录形态：

```text
/home/hyde/projects/ptm-work/<feature-slug>/
```

其中 `<feature-slug>` 使用特性英文短名或拼音短名，便于从项目目录名推导默认特性名。若用户显式提供特性名，以用户提供值为准；其次读取需求标题；最后才使用项目目录最后一级名称。
...
### 4.3 Topology Modeling（CR-008）

对依赖明确组网的场景，`scenario-discovery` 会在 `analysis/scenarios/<scene-id>/` 下同步输出：
...
| 目标解析 | Action Source 的 `target` 应能解析到 `DUT<n>` 或 `DUT<n>.Port<n>` |

### 4.4 MCP 只读 staged query / tri-state 契约
...
- **输入**：目录结构、只读 MCP、REST API 配置、CLI/工具说明
- **输出**：场景链、`topology.mmd/yaml`、动作源目录、知识引用、缺口项
...
- **输入**：全部分析与设计产物
- **输出**：`delivery/<特性名>特性测试方案.md`、`delivery/<特性名>特性测试用例.md`
- **示例**：`@ptm-tde 输出最终交付文档`
...
`deliverable-renderer` 会在 `delivery/<特性名>特性测试用例.md` 中保留结构化检索字段，核心字段包括：
...
| 名称 | 类型 | 主要用法 | 用途 | 适用场景 |
|---|---|---|---|---|
| `ptm-team` 统一安装工具 | cli-tool | 安装、升级、卸载和平台投影 ptm-tde 组件 | 组件生命周期管理 | 安装与初始化 |
| `excel_coupling_tool.py` | tool-method | 读取耦合矩阵基线 | F 分析耦合输入 | 有耦合矩阵的标准/复杂分析 |
| `mcp-query-client.py / MCP Gateway` | rest-api / tool-method | 只读 staged query | 领域/特性/功能知识引用 | 有知识库入口的标准/复杂分析 |
| 用户提供 REST API | rest-api | 作为原子操作或验证动作源 | 场景链建模与观察点落地 | 复杂场景 |
| 用户提供 CLI 工具 | cli-tool | 通过 CLI 调用现有测试工具 | 支撑原子操作和结果观察 | 复杂场景 |
| `topology.mmd` / `topology.yaml` | model artifact | 固化设备/端口/链路关系 | 场景确认与组网回链 | 有明确组网要求的标准/复杂分析 |
| `case-retriever` | tool-method | 消费最终测试用例总表的结构化字段 | 交付后反查用例 | 交付与维护阶段 |
...
Agent：解析目录并请求确认
用户：确认
Agent：识别原子操作与动作源，发现 2 个工具缺口，请求补充
...
| 交付物 | 说明 |
|---|---|
| `delivery/<特性名>特性测试方案.md` | 汇总需求、Scenario Chain、MFQ 分析、设计计划、覆盖率和工具缺口 |
| `delivery/<特性名>特性测试用例.md` | 汇总所有物理用例，并保留 requirement / LC / feature tag / trace refs 等检索字段 |

`analysis/` 与 `design/` 下的 Topology、PPDCS 设计过程、PC 设计过程属于过程资产；测试方案引用并汇总这些资产，但不把它们作为独立最终交付文档。
...
这是新的特性项目运行约定：`input/` 保护原始输入，分析、设计、检查点、交付和状态分别写入项目根目录下的专用目录，避免再套一层 `.output/`。
...
因为很多动作源、观察点和工具分析都依赖明确的设备/端口归属；把 Topology 放到场景确认阶段，能更早暴露组网缺口，避免后续设计链回退。
...
| 缺口类型 | 影响项 | 严重程度 | 建议处理 |
|---------|--------|---------|---------|
| 无 | 当前手册已覆盖组件定位、特性项目运行目录、Scenario Chain + Topology、MCP 只读三态、五类 PPDCS、检索闭环、工具与动作源分析口径 | 低 | 后续若 ptm-team 统一安装手册变更或新增 Topology 校验器，再同步扩展本手册 |
