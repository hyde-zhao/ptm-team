...
发布后真实使用中发现的问题，不建议靠口头描述或手工整理模板回传。`ptm-tde` 在交付或真实运行结束时会通过 `tde-feedback` Skill 询问“本次 ptm-tde 使用是否有问题需要反馈？”。使用者确认有问题后，Skill 会调用 `script/field_feedback.py collect / submit / publish` 生成标准采集包；`collect` / `submit` 会自动登记 `RUN-EXEC-*` 并写入 `collection_path`，`submit` 还会同步 GitLab 并写入 `published_path`。维护侧用 `pull` 拉回原始材料后，再基于 RUN-EXEC 生成 ISSUE、覆盖盲区分析和质量看板。
...
其中选择“有问题，采集并上传”或“上传已有采集包”即表示授权 `--push` 到默认反馈仓库；选择“有问题，仅采集”时只生成 `COLLECT-*` 包并自动登记本地 `RUN-EXEC-*`，不提交、不推送。
...
维护侧分析时，以自动生成的 `RUN-EXEC-*` 为入口，并通过其中的 `collection_path` / `published_path` 回查原始材料；失败项再生成 ISSUE，补 coverage_status、category、severity、stage 和 regression_asset，最后刷新 `docs/quality/FIELD-QUALITY-DASHBOARD.md`。脚本完整命令见 `script/README.md`。

### 6.4 meta-flow 评估侧消费反馈与真实运行样本

ptm-team 侧保留 ptm-tde 的领域实例，meta-flow 侧提供通用 runner / grader / release-check 能力。当前 ptm-tde 的评估配置已经把真实运行样本、反馈源和发布门禁接入 `evals/WORKFLOW-EVAL.yaml`：

- `feedback_sources`：声明本地 `process/field-feedback/collections` 和 GitLab `ptm-team-feedback.git:tde-feedback`。
- `runtime_artifact`：检查 BGP4+ partial workspace 与 policy_route full workspace。
- `install_mapping`：检查 `/home/hyde/projects/ptm-tde/test/.claude` 安装面包含 `ptm-tde` 和 `tde-feedback`。
- `RELEASE-CHECK-PROFILE.yaml`：声明 release profile 的 runtime、feedback、blocking case 和 regression asset 门禁。
...
若需要直接从 GitLab 拉取反馈材料，必须显式授权 Git 读取：
...
`suite-health` 只表示长期健康趋势，`release-check` 才是发布门禁。`release-check` 会消费 CASE 中的 `blocking`、`severity`、`coverage_status`、`runtime_required`、`source_issue` 和 `regression_asset` 字段；新增 runtime 或反馈缺陷后，必须同步补 CASE 元数据，否则发布门禁无法稳定判断。
...
### Q10：别人真实使用后的问题怎么反馈？
让使用者触发 `tde-feedback` Skill，先回答是否有问题反馈；确认有问题后，Skill 会在工作区执行 `field_feedback.py submit` 生成 `COLLECT-*` 采集包、自动登记 `RUN-EXEC-*`，并推送到 `ptm-team-feedback.git` 的 `main` 分支；也可以先执行 `collect`，此时只生成本地 `COLLECT-* + RUN-EXEC-*`，确认材料后再执行 `publish`。维护者在 `ptm-team` 执行 `field_feedback.py pull --root .` 拉回原始材料，然后基于 RUN-EXEC 生成 ISSUE、覆盖盲区分析和回归资产。
...
| 日期 | 变更 | 处理人 |
|------|------|--------|
| 2026-06-17 | CR-022：ptm-team 接入 meta-flow `runtime_artifact`、`feedback_sources`、`install_mapping` 和 `release-check` profile 消费端配置。 | Codex |
| 2026-06-17 | CR-020：`tde-feedback collect / submit` 自动登记 `RUN-EXEC`，并在运行记录中绑定 `collection_path` / `published_path`。 | Codex |
