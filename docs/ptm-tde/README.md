...
- `test-point-integrator` 从 `analysis/scenarios/confirmed-scenarios.md` 绑定真实组网对象并写入 LC `topology_bindings`。
...
| Topology Bindings | 真实端口是否来自已确认场景，并可回链到 `confirmed-scenarios.md` |
...
公共因子库主库不保存在项目 `analysis/` 中；项目只在 `analysis/factor-usage/` 记录本次使用的库版本、因子绑定和候选回流建议。真实设备、真实端口和 link 实例不进入 `analysis/factor-usage/`，由 `analysis/scenarios/confirmed-scenarios.md` 和 LC `topology_bindings` 承载。
...
- CP02 已扩展为 auto + manual：自动检查输入分类、再发现、Seed-to-Scenario Mapping、Topology Catalog、atomic-ops 唯一口径、Operation Path 和缺口分类。
...
| 项目消费记录 | `analysis/factor-usage/` |

安装 `ptm-tde` agent 时，ptm-team 安装器同步安装关联 factor libraries；卸载时按 manifest 中的 `installed_for` 引用关系删除独占资源，避免误删仍被其他组件使用的公共库。
