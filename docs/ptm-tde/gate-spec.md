...
| 3 | 原子操作可用 | 全局命令 `atomic-ops` 可执行；`atomic-ops list --format json` 返回全部操作清单（含 tags + parameters_summary） | 提示用户补充；WARNING 不阻断，m-analyzer 降级精确匹配 |
| 4 | 防火墙 topo 可用 | `input/` 下存在 topo 文件，或 wiki 可找到 topo | 提示用户提供 |
| 5 | 耦合矩阵可用 | `input/` 下存在耦合矩阵，或 wiki 可找到 | 提示用户提供 |
...
| 5 | Topology Catalog | 依赖组网的场景均有 `topology_ref`、来源、Mermaid、设备/端口/链路表 | 缺失拓扑时阻断 |
| 6 | atomic-ops 唯一口径 | `source_type=atomic-ops`；`action_source_ref` 引用 atomic-ops `op_id` | 出现旧口径时阻断 |
...
| 12 | Tool Gap | 未满足的 atomic-ops 进入 `Tool Abstraction Draft` 或 `confirmation_gaps` | 缺口未记录时阻断 |
...
| M9 | 原子操作匹配完整性 | `candidate-atomic-ops.yaml` 中每个候选记录了 match_attempt（L1-L4 + score）；atomic-op-resolution-report.md 存在 | 缺失 match_attempt 时回到 M 分析补语义匹配 |
