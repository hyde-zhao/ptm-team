...
| `mfq/factor-usage/factor-library-lock.yaml` | 运行时因子库版本快照（新增） |
...
- **v3.0 新增 4**：Step 1.5 扫描完整性校验必须在进入 Step 2 前完成。N_scanned < N_registered 时必须 HARD-STOP 阻断，不得以降级方式继续。缺失库可能导致候选因子大量假阳性
...
- [ ] 输出 9 个文件：`test-points.md` / `ppdcs-annotation.md` / `test-objects-factors.md` / `scenario-tsp-coverage.md` / `tsp/<M>-tsp.md` / `factor-resolution-report.md` / `candidate-factor-proposals.yaml` / `candidate-atomic-ops.yaml` / `factor-library-lock.yaml`
