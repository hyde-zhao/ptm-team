...
【1.6.3 构建内存索引】
1. 构建 op_id → {description, tags[], params[name,type], device_type, since_version} 的字典索引
2. 为所有 op_id 预计算分词结果（按下划线/驼峰/数字边界分词 + 词根分解）供 Step 2C 使用
  ...
  对场景中的每个操作步骤（precondition_operations + atomic_operations + minimal_logic_chain）：
    ...
    **语义匹配算法（加权分词重叠）**：
    ...
    1. 候选分词：
       ...
       对每个 token 做词根分解（如 subinterface→[sub, interface]、lag→[link, aggregation]）
    ...
    3. 对索引中的每个 op_id 计算总分：
       ...
       total_score   = op_id_score + desc_score + tags_score + params_score
    ...
    5. 同义词表（内嵌）：
       以下领域映射用于辅助分词命中（扩展 token 集合）：
         subinterface → interface, sub-if
         trunk → interface
         lag → link-aggregation
         bond → link-aggregation
         bvi → bridge, interface
         vlan → interface, layer2
         acl → access-control
