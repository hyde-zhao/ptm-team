...
| 消费数据 | 字段 | 用途 |
|---------|------|------|
| `resource/factor-libraries/index.yaml` | libraries[].library_id / status / path / version | 库注册清单 |
| `resource/component-resource-links.yaml` | library_id / install_policy | 确认全量消费意图 |
...
```
【1.5.1 读取库索引】
1. 读取 resource/factor-libraries/index.yaml → 获取全部已注册库的 library_id / status / path / version
2. 读取 resource/component-resource-links.yaml → 确认 ptm-tde 组件的 library_id=all、install_policy=required
...
> 在进入原子操作支撑检查之前，查询 atomic-ops CLI 获取全部已有操作的完整元数据，构建 op_id 索引。复用 Step 1.5 的资源发现模式。

**📥 消费**：

| 消费数据 | 来源 | 用途 |
|---------|------|------|
| atomic-ops CLI | `atomic-ops list --format json` | 获取全部 79 个 OperationSummary（op_id/description/tags/parameters_summary/device_type） |
...
```
【1.6.1 运行 CLI 查询】
1. 执行 atomic-ops list --format json
...
对每个 OperationSummary 提取：
  ...
  - aliases[]（如 ["subinterface","trunk","lag","bond","bvi"]）— 操作的业务别名，来自 atomic-ops 仓库定义
...
【1.6.4 Lock 文件管理】
1. 检查 mfq/atomic-op-usage/atomic-op-lock.yaml 是否存在
2. 若不存在 → 创建，记录 CLI commit_sha + total_ops + timestamp
3. 若存在 → 校验一致性（比较 commit_sha 和 total_ops）
   - 一致 → OK
   - 不一致 → ⚠️ WARNING："atomic-ops 版本已变更（旧: X ops @ sha1，新: Y ops @ sha2）"，继续
4. 写入前校验：mfq/atomic-op-usage/ 父目录存在且为目录（遵循 STOP-04）

【1.6.5 CLI 不可用降级】
1. atomic-ops 命令未安装或执行失败：
   → ⚠️ WARNING："atomic-ops CLI 不可用，原子操作支撑检查将仅使用精确匹配（action_source_ref）"
...
| `mfq/atomic-op-usage/atomic-op-lock.yaml` | CLI 版本快照（commit_sha + total_ops + timestamp） |
...
对每个已确认的场景（Scenario Chain）：
  对场景中的每个操作步骤（precondition_operations + atomic_operations + minimal_logic_chain）：
    ...
    > 使用 Step 1.6 构建的 atomic-ops 内存索引，对每个操作步骤执行四级匹配。
    ...
    **语义匹配算法（加权分词重叠）**：
    ...
    2. 权重参数（可调常量）：
       ...
       ALIASES_WEIGHT     = 1.5   # aliases 分词命中权重（来自 atomic-ops 定义）
    ...
    5. 操作别名（aliases）消费规则：
       别名数据来自 Step 1.6 构建的 atomic-ops 内存索引中的 aliases 字段，
       由 atomic-ops 仓库统一定义和维护，m-analyzer 不自行维护同义词表。
       常用别名示例（供理解，实际以 atomic-ops 返回为准）：
         fw_config_interface.aliases → [subinterface, trunk, lag, bond, bvi, vlan-interface]
         fw_config_nat.aliases       → [snat, dnat, source-nat, destination-nat]
         fw_config_acl_policy.aliases → [access-list, firewall-policy]
    ```
...
| `mfq/atomic-op-usage/atomic-op-lock.yaml` | atomic-ops CLI 版本快照（新增，CR-016） |
...
- M 分析是公共因子库首个强制消费者；提取测试因子前必须先执行 Step 1.5（因子库清单加载），读取 `resource/factor-libraries/index.yaml` 发现全部已注册库，遍历加载各库 `factor-library.yaml`，构建内存索引。
- 公共库发现方式（v3.0+）：从 `resource/factor-libraries/index.yaml` 读取库注册清单 → 遍历每个已注册库（无论 active/candidate）→ 加载其 `factor-library.yaml` → 构建 factor_id → meta 内存索引。不再使用旧版三层路径回退查找。
...
- **v3.0 新增 5**：Step 1.6 atomic-ops CLI 不可用时必须降级（WARNING + 回退精确匹配），不得阻断流程。语义匹配是增强而非替代——精确匹配（L1）始终可用
...
- [ ] 输出 10 个文件：`test-points.md` / `ppdcs-annotation.md` / `test-objects-factors.md` / `scenario-tsp-coverage.md` / `tsp/<M>-tsp.md` / `factor-resolution-report.md` / `candidate-factor-proposals.yaml` / `candidate-atomic-ops.yaml` / `factor-library-lock.yaml` / `atomic-op-lock.yaml`
...
- [ ] Step 1.6 atomic-ops CLI 查询成功或已降级（CLI 不可用时 WARNING + 回退）
