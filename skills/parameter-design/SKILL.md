...
每个 `case_steps[].atomic_op.op_id` 必须逐字来自当前 LC / PC 的 `action_source_refs`。不得使用示例中的通用操作、相近语义操作或 IPv4/IPv6 错配操作替代上游已确认 `op_id`；若没有可用 `op_id`，必须输出 `pc_step_contract_gap` 并保持 `fact_status=needs-confirmation`。
...
|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|------------|------------|--------|---------|----------|
| 日志中心 | 日志管理 | 权限管理 | 管理员修改通知级别日志 | PC-LOG-PRM-001 | P1 | 单台防火墙 | | 管理员已登录；存在通知级别日志 | 1. 选择通知级别日志<br>执行对象：DUT<br>原子操作：select-log-record level=notice<br>2. 以管理员角色修改日志级别并保存<br>执行对象：DUT<br>原子操作：update-log-level role=admin level=notice action=save | 1.允许修改<br>2.保存成功 | V60R001C01 | | 权限,判定表 | 功能 | 否 |
| 日志中心 | 日志管理 | 权限管理 | 管理员修改紧急级别日志[待确认] | PC-LOG-PRM-006 | P2 | 单台防火墙 | | 管理员已登录；存在紧急级别日志 | 1. 选择紧急级别日志<br>执行对象：DUT<br>原子操作：select-log-record level=critical<br>2. 以管理员角色尝试修改紧急级别日志<br>执行对象：DUT<br>原子操作：update-log-level role=admin level=critical action=modify | 1.预期拒绝；具体错误码[待确认] | V60R001C01 | | 权限,待确认 | 功能 | 否 |
```
...
- PC `测试步骤*` 不能只写“执行修改并保存”或只写 `op_id`；必须同时给出步骤名称与 `原子操作：<op_id> <args...>`。
