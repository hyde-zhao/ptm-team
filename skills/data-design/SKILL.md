...
**case_title 生成规则**（data-design 方法特定）：

- 格式：`<等价类/边界场景描述>`，如 `入接口配置为物理口`、`入接口为超长接口名(边界越界)`
- 禁止用 `physical_case_id` 占位；禁止保留 data_row 内部编号（如 `DR-001:`）作为标题主体
- 同 LC 内多条 PC 的 case_title 必须可区分（不同等价类或不同边界场景）
...
|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|---------|------------|------------|--------|---------|----------|
| 日志中心 | 配置管理 | 日志备份配置 | 日志保存天数最小值配置 | PC-CFG-DAT-001 | P2 | 单台防火墙 | | 管理员已登录 | 1. 配置日志保存天数为最小边界值 1<br>执行对象：DUT<br>原子操作：config-log-retention days=1<br>2. 保存并刷新日志备份配置<br>执行对象：DUT<br>原子操作：commit-log-backup-config refresh=true | 1.保存成功<br>2.页面显示保存天数为 1 | V60R001C01 | | 边界值,等价类 | 功能 | 否 |
| 日志中心 | 配置管理 | 日志备份配置 | 备份路径非法值校验[待确认] | PC-CFG-DAT-009 | P3 | 单台防火墙 | | 管理员已登录 | 1. 输入待确认的非法备份路径<br>执行对象：DUT<br>原子操作：config-log-backup-path path=[待确认]<br>2. 保存备份路径配置<br>执行对象：DUT<br>原子操作：commit-log-backup-config refresh=false | 1.系统路径校验规则[待确认] | V60R001C01 | | 待确认,数据边界 | 功能 | 否 |
```
...
- PC `测试步骤*` 不能只写数据取值或只写 `op_id`；必须同时给出步骤名称与 `原子操作：<op_id> <args...>`。
