...
description: >-
  ...
  重新发现并收敛可直接支撑逻辑用例写作的部署型 Scenario Chain / Topology / ptm-atomic / Knowledge Reference。
...
生成**准确优先、禁止脑补**的部署型场景发现结果。功能初稿不可直接作为最终场景；必须经过输入文档类型识别、场景再发现、头脑风暴、范围收敛、Topology 锚定、ptm-atomic 原子操作映射、Seed-to-Scenario Mapping 和输出质量检查。
...
- ptm-atomic（项目内唯一原子操作引用对象，直接引用 `op_id`）
...
- 项目存在 `.input/TGFW测试组网图集合.md` 或 `resource/network-topology/topology-collection.md`，需要基于 TGFW 测试组网图集合选择、映射和验证拓扑。
- 项目内统一使用 ptm-atomic 原子操作，需要直接引用 `op_id`，并暴露能力状态和调用/观测契约。
...
- 不发明 API、CLI、ptm-atomic、组网、设备端口、链路、性能容量指标或项目事实。
...
7. 用户补充的 ptm-atomic 能力说明；REST API、CLI、tool-method 只能作为 ptm-atomic 的底层调用契约或实现通道，不作为独立引用类型。
...
| `functional_scenario_seed` | 以功能点、功能流程、操作步骤或用例雏形为主，缺少部署维度、Topology、ptm-atomic 或验证链 | 进入场景再发现，不得直接转最终场景 |
| `deployment_scenario_draft` | 已包含部署目标、Topology、场景矩阵、ptm-atomic、Knowledge Reference 或 Confirmation Gaps，但仍有缺口 | 进入补全、校验和收敛 |
| `confirmed_scenario_artifact` | 用户或项目显式确认，且包含确认记录、稳定场景编号、拓扑和 ptm-atomic | 仅做一致性复核；新增内容必须标记为候选或缺口 |
...
7. **ptm-atomic 显式建模**：项目内统一使用 ptm-atomic 原子操作；REST API、CLI Tool、Tool Method 只能作为 ptm-atomic 的 `invoke_contract`、`observe_contract` 或实现通道描述。
...
1. 读取用户输入、`kym/feature-input/`、功能种子、部署型样例、TGFW 组网集合和 ptm-atomic 说明。
...
| interface shape | UI、REST API、CLI、tool-method 与 ptm-atomic 的边界是什么；哪些接口只能作为 ptm-atomic 的底层实现通道 |
...
**独立小上下文**：每个场景的 Scenario Chain、ptm-atomic 和 Knowledge Reference 应为封闭的独立上下文，可被下游 M/F/Q 分析独立消费而无需重新加载全部场景。
...
组网拓扑来源按优先级：① `resource/network-topology/topology-collection.md`（或 `~/.ptm-team/resource/network-topology/`） → ② `.input/TGFW测试组网图集合.md` → ③ wiki。
...
4. ptm-atomic 的 `target` 若指向设备或端口，必须可解析到 `DUT<n>` 或 `DUT<n>.Port<n>`。
...
- 每个 scenario 必须在 `abnormal_path` 后、`exit_action` 前单独输出 `minimal_logic_chain` 小节；不得只在文档总表或质量检查中提到。

`minimal_logic_chain` 建议表格格式：

| 链路节点 | 来源步骤 | necessity | action_source_refs | 说明 |
|----------|----------|-----------|--------------------|------|
| L1 | normal_path.1 | 必要 | `fw_config_interface_ipv4` | 接口地址准备 |
| L2 | normal_path.3 | 至少选择一项 | `fw_config_policy_route_src` / `fw_config_policy_route_dst` | 策略匹配条件至少选择一项 |
...
| `action_source_refs` | 本步骤依赖的 ptm-atomic `op_id`，必须逐步骤填写，不能只在场景级总表填写 |
| `atomic_op` | 本步骤的主原子操作；至少包含可回链到 `action_source_refs` 的 `op_id` |
...
|------|--------|------|--------|------------------------------------|------|
| 3 | 3.1<br>3.2 | 配置匹配对象：<br>配置地址对象<br>配置服务对象 | 可选 | `fw_create_addr_object`<br>`fw_create_service_object` | 仅当策略路由规则引用地址对象或服务对象时执行 |
| 6 | 6.1<br>6.2<br>6.3<br>6.4 | 配置策略路由匹配条件：<br>配置源地址匹配<br>配置目的地址匹配<br>配置服务匹配<br>配置入接口匹配 | 至少选择一项 | `fw_config_policy_route_src`<br>`fw_config_policy_route_dst`<br>`fw_config_policy_route_service`<br>`fw_config_policy_route_in_if` | 同一条策略路由规则的匹配条件不能全部跳过 |
...
| `action_source_refs` | 触发或观测该异常的 ptm-atomic `op_id`，必须逐异常项填写 |
| `atomic_op` | 该异常项的主原子操作；至少包含可回链到 `action_source_refs` 的 `op_id` |
...
| 字段 | 说明 |
|------|------|
| `op_id` | 操作编号；若来自 ptm-atomic，直接复用或引用原始 `op_id` |
| `phase` | `precondition` / `main-flow` / `exit` |
| `channel` | UI / REST API / CLI / Tool Method / Manual / ptm-atomic；非 ptm-atomic 通道必须绑定到某个 ptm-atomic 的调用或观测契约 |
| `action_object` | 操作对象 |
| `input_params` | 输入参数 |
| `observation_point` | 该操作完成后的观察点 |
| `expected_state` | 该操作完成后的预期状态 |
| `action_source_ref` | 关联 ptm-atomic；直接引用 `op_id` |

### 步骤 8：建模 ptm-atomic / Knowledge Reference / Tool 输出

#### ptm-atomic

对所有 ptm-atomic 进行显式建模。本项目中 **ptm-atomic 是唯一原子操作引用对象**。REST API、CLI、tool-method 不是独立引用类型，只能作为 ptm-atomic 的底层调用契约、观测契约或实现通道。

- `source_type` 固定使用 `ptm-atomic`。
- `action_source_ref` 直接引用 ptm-atomic 的 `op_id`。
...
| 字段 | 说明 |
|------|------|
| `action_source_ref` | ptm-atomic 引用；必须直接填 ptm-atomic `op_id` |
| `source_type` | 固定为 `ptm-atomic` |
| `capability_status` | `ready` / `gap` / `unknown` |
| `invoke_contract` | ptm-atomic 的调用入口、参数、前置条件；可说明底层 REST API / CLI / tool-method，但不得把它们建模为独立引用对象 |
| `observe_contract` | ptm-atomic 调用后如何观测结果；可说明底层观测通道 |
| `scenario_refs` | 关联场景 |

UI/Manual 操作可以出现在 `channel` 中；若需要自动化或工具化消费，必须映射到 ptm-atomic，或进入 Tool Abstraction Draft / Confirmation Gaps。
...
|------|------|
| `tool_name` | 待抽象工具名称 |
| `target_interface` | API / CLI / method / ptm-atomic wrapper |
...
> **⛔ HARD-STOP**：每完成一个场景的 Scenario Chain、Topology、ptm-atomic 和 Knowledge Reference 分析后，必须暂停并请求用户确认，不得自动进入下一个场景。
...
2. **ptm-tde Scenario Discovery Structured Supplement**：面向工具化消费，包含分类、矩阵、映射、ptm-atomic、知识引用、工具草案、缺口和质量检查。
...
| Atomic Operation Summary | 原子操作摘要和 ptm-atomic 关联 |
| ptm-atomic | 原子操作清单：`action_source_ref / source_type=ptm-atomic / capability_status / invoke_contract / observe_contract / scenario_refs` |
...
| `preconditions` | 前置条件 |
| `action_source_refs` | 场景级 ptm-atomic `op_id` 汇总；必须覆盖 `normal_path`、`abnormal_path`、`minimal_logic_chain` 和 `exit_action` 引用的全部 op_id |
| `normal_path` | 正常路径；必须包含 `step_id / sub_step_ids / operation / necessity / action_source_refs / atomic_op.op_id / description` |
| `abnormal_path` | 异常路径；必须包含 `abnormal_item / related_normal_steps / input_or_state / action_source_refs / atomic_op.op_id / expected_handling`；无异常路径时必须说明不适用理由 |
| `minimal_logic_chain` | 最小逻辑链；必须逐 scenario 输出，保留必要 / 可选 / 至少选择一项语义，并回链 `normal_path` 步骤与 `action_source_refs` |
| `atomic_operations` | 主流程原子操作 |
...
| `action_sources` | ptm-atomic 原子操作清单 |
...
- ptm-atomic 是唯一原子操作引用对象，`action_source_ref` 必须直接引用 `op_id`。
- `source_type` 必须固定为 `ptm-atomic`，不得出现 `rest-api`、`cli-tool`、`tool-method`。
...
- 每个 scenario 必须输出显式场景级 `action_source_refs` 字段；该字段必须覆盖本场景正常路径、异常路径、最小逻辑链和退出动作中引用的全部 `op_id`。
- `normal_path` 和 `abnormal_path` 中每个步骤都必须包含步骤级 `action_source_ref(s)`、`atomic_op` 或 `op_id`，并能回链场景级 `action_source_refs`；不得只在场景级列一个 atomic operation 总表。
- 每个 scenario 必须包含独立的 `minimal_logic_chain` 小节；GATE-2 会阻断只写 normal/abnormal path 而缺少最小逻辑链的场景。
- 发现现有 ptm-atomic 不能覆盖的操作时，只能输出为新增 / 候选原子操作并进入 GATE-2 人工确认项；不得静默标记为 confirmed。
...
- [ ] 每个 confirmed scenario 的正常路径和异常路径均被覆盖；无异常路径时已说明不适用理由。
- [ ] 每个 scenario 均有显式场景级 `action_source_refs` 汇总，且覆盖所有步骤级 op_id。
- [ ] `normal_path` 已区分大步骤和子步骤；多动作步骤未被压缩成不可追溯描述。
- [ ] `normal_path` 和 `abnormal_path` 每个步骤均有步骤级原子操作引用，并回链场景级 `action_source_refs`。
...
- [ ] 每个 scenario 均有独立 `minimal_logic_chain` 小节。
...
- [ ] 已统一按 ptm-atomic 建模，`source_type=ptm-atomic`，`action_source_ref` 直接引用 `op_id`。
- [ ] 未将 REST API、CLI 或 tool-method 作为独立引用类型输出；它们只出现在 ptm-atomic 调用/观测契约中。
...
- 对用户未提供的 ptm-atomic 契约不能自行补全为可执行细节；REST API / CLI / tool-method 细节只能作为 ptm-atomic 底层契约补充。
...
- `confirmed-scenarios.md` 只有文件级 `normal_path` / `abnormal_path` 字段名不算完整；GATE-2 会逐 `scenario_id` / 场景标题阻断缺少正常链或异常链的场景。
- 场景级 `atomic_operations` 不能替代步骤级 `action_source_ref(s)`；GATE-2 会阻断缺少步骤级 op 的正常链或异常链条目。
- Markdown 表格列名出现 `action_source_refs` 不等于场景级字段；每个 scenario 必须有独立字段行或 YAML key，例如 `action_source_refs: [fw_config_xxx, tg_send_xxx]`。
- 输出质量检查里勾选 `minimal_logic_chain` 不等于逐场景输出最小逻辑链；每个 scenario 正文必须有可解析的小节或字段。
...
- [ ] 每个场景包含显式 `action_source_refs`，且能覆盖本场景所有步骤级 `op_id`。
- [ ] 每个场景的 `normal_path` 包含 `step_id / sub_step_ids / operation / necessity / action_source_refs / atomic_op.op_id / description`，且必要性取值规范。
- [ ] 每条正常路径和异常路径步骤都包含可回链的原子操作引用。
- [ ] 每个场景正文均包含独立 `minimal_logic_chain`，并可回链 `normal_path` 步骤。
- [ ] 每条异常路径包含 `related_normal_steps` 或说明无法追溯的来源。
- [ ] ptm-atomic 是唯一原子操作引用对象；REST API / CLI / tool-method 未作为独立引用类型输出。
