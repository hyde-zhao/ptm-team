...
| STOP-06 | 交互降级 | 当 AskUserQuestion 工具不可用或选项超过 4 个且无法语义拆分时，必须回退到 STOP-05 文本标记。禁止在工具不可用时仍尝试 AskUserQuestion |

### 人工确认交互方式（AskUserQuestion 增强）

GATE-2 / GATE-3 / GATE-4 的人工确认在 Claude Code 环境下优先使用 `AskUserQuestion` 工具：
...
若 AskUserQuestion 不可用，回退到标准 HARD-STOP 文本协议（`approve` / `修改:` / `reject`）。
