...
当前脚本执行的是 **machine-baseline + 字段级结构检查**：除目录和文件存在性外，还会检查关键字段是否存在，例如 GATE-2 的 `normal_path / abnormal_path / action_source_refs / confirmation_gaps`，GATE-3 的 CAE、trace、`factor_bindings / topology_bindings` 和 PPDCS 方法字段，GATE-4 的 PC 16 列表格和交付 trace 字段。语义质量、风险接受和业务合理性仍由独立 manual Gate 草稿承载。
...
| P2 | PC 文件完整 | `ppdcs/pc/` 下每个 LC 都有物理用例文件，16 列格式正确 | 缺失时回到 PC 生成 |
...
| P6 | 交付物完整 | `ppdcs/delivery/` 包含且仅包含测试方案和测试用例两个文件 | 缺失或多余时修正交付物 |
| P7 | 交付物字段保留 | 交付物保留 `topology_bindings / topology_role / source / fact_status` | 字段缺失时修正渲染逻辑 |
