...
| M10 | 候选测试因子显式确认状态 | 存在候选因子来源时，`mfq/candidates/factor-candidates.md` 或等价文件必须记录最终 `decision=confirmed/rejected/modified`；`pending-review` 只表示待评审，不算通过 | 缺确认时回到候选汇总确认 |
| M11 | 候选原子操作显式确认状态 | 存在候选原子操作来源时，`mfq/candidates/ptm-atomic-candidates.md` 或等价文件必须记录最终 `decision=confirmed/rejected/modified`；`pending-review` 只表示待评审，不算通过 | 缺确认时回到候选汇总确认 |
...
| 状态更新 | `process/STATE.yaml` 在 GATE-3 机器基线 PASS 后保持 `current_phase: mfq`、`current_step: mfq-exit`，等待人工确认；不得仅凭机器 PASS 推进到 `ppdcs` |
